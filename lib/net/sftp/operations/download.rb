...
module Net; module SFTP; module Operations
      ...
      class Download
        ...
        # When downloading, should an existing file with same mtime & size
        # should be kept
          ...
          if response.eof?
            ...
            response[:names].each do |item|
              ...
              if dont_overwrite_if_same_file? &&
                  ...
                  item.attributes.mtime == ::File.mtime(local_file).to_i &&
                  item.attributes.size == ::File.size(local_file)
                  update_progress(:open, entry)
                  update_progress(:close, entry)
                next
              end

              stack << Entry.new(::File.join(entry.remote, item.name),
                local_file,
                item.directory? || (item.symlink? && follow_symlinks?),
