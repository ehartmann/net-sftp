...
module Net; module SFTP; module Operations
      ...
      class Download
        ...
        # When downloading, if the local file has the same mtime and same size, should
        # we prevent from downloading ?
        ...
        def on_readdir(response)
          ...
          if response.eof?
            ...
            response[:names].each do |item|
              ...
              next if dont_overwrite_if_same_file && ::File.exist?(local_file) && item.attributes.mtime == ::File.mtime(local_file).to_i && item.attributes.size == ::File.size(local_file)

              stack << Entry.new(
                ::File.join(entry.remote, item.name),
                local_file, item.directory? || (item.symlink? && follow_symlinks?),
                item.attributes.size,
                nil,
                nil,
                nil,
                item.attributes.atime,
                item.attributes.mtime
              )
            end
        ...
        def on_read(response)
          ...
          if response.eof?
            ...
            entry.sink.close
            ::File.utime(Time.at(entry.atime), Time.at(entry.mtime), entry.local) if preserve_time? && entry.mtime
