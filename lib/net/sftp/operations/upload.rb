...
module Net; module SFTP; module Operations
      ...
      class Upload
        ...
        def preserve_time?
          options[:preserve_time]
        end

        # When uploading, if the local file has the same mtime and same size as the remote file,
        # prevent from uploading
        def dont_overwrite_if_same_file?
          options[:dont_overwrite_if_same_file]
        end
        ...
        # uploaded.
        LiveFile = Struct.new(:local, :remote, :io, :size, :handle, :ctime, :mtime, :atime)
        ...
        def open_file(local, remote)
          ...
          if skip_file
            ...
            metafile = LiveFile.new(name, remote, file, size, nil, ctime, mtime, atime)
            update_progress(:open, metafile)

            # Should only works with SFTP version >= 5
            #request = sftp.open(remote, "w", {:atime => file.atime.to_i, :mtime => file.mtime.to_i, :ctime => file.ctime.to_i}, &method(:on_open))
        ...
        def on_mkdir(response)
          ...
          unless response.ok?
            sftp.stat(remote) do |response|
              raise StatusException.new(response, "mkdir #{dir}") if !response.ok? && (reponse.attributes.directory? || response.attributes.symlink?)
            end
          end
        ...
        def on_close(response)
          @active -= 1
          file = response.request[:file]
          if preserve_time? && file.atime && file.ctime && file.mtime
            sftp.setstat(file.remote,
              :mtime => file.mtime.to_i,
              :atime => file.atime.to_i,
              :ctime => file.ctime.to_i) do |response2|
              raise StatusException.new(response2, "Changing time for #{file.remote}") unless response2.ok?
            end
          end
